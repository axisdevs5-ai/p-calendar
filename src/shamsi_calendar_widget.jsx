...
export default function ShamsiCalendarWidget() {
  ...
  const resetDefaults = () => {
    ...
    useLayoutEffect(() => {
    // اطمینان حاصل می‌کنیم که این کد فقط در محیط Electron اجرا می‌شود
    const { ipcRenderer } = window.require ? window.require('electron') : {};
    if (ipcRenderer && mainDivRef.current) {
      // یک наблюдатель (Observer) ایجاد می‌کنیم تا هر تغییری در اندازه را تشخیص دهد
      const observer = new ResizeObserver(entries => {
        const entry = entries[0];
        if (entry) {
          const { width, height } = entry.contentRect;
          // اندازه جدید را به پروسه اصلی Electron می‌فرستیم
      ...
      // مشاهده‌ی div اصلی را شروع می‌کنیم
      observer.observe(mainDivRef.current);

      // در زمان unmount کامپوننت، مشاهده را متوقف می‌کنیم
  ...
  const mainCardClasses = `grid grid-cols-2 gap-0 rounded-[32px] overflow-hidden shadow-2xl relative transition-all duration-300 w-[612px] max-w-2xl ${glassTheme ? 'backdrop-blur-2xl bg-black/30 border border-white/10 text-neutral-100' : lightBg ? 'bg-white text-neutral-900' : 'bg-neutral-900 text-neutral-200'}`;
  ...
  return (
    <div className={`${lightBg ? 'light-theme' : ''} ${!lightBg && !glassTheme ? 'dark-theme' : ''} ${glassTheme ? 'glass-theme' : ''}`}>
     <style>{`
  html, body {
  ...
  .draggable-area {
    -webkit-app-region: drag; /* به الکترون میگه این بخش قابل درگه */
    cursor: grab; /* شکل موس رو تغییر میده که کاربر بفهمه قابل درگه */
  }

  /* هر عنصری که یکی از این مشخصات را داشته باشد، قابل کلیک خواهد بود */
  .draggable-area button,
  .draggable-area input,
  .draggable-area select,
  .draggable-area textarea,
  .draggable-area a,
  .draggable-area [role="button"] {
    -webkit-app-region: no-drag; /* قابلیت درگ رو برای این عناصر غیرفعال میکنه */
    cursor: default; /* شکل موس رو برای این عناصر به حالت عادی برمیگردونه */
  }

  /* Styles for custom scrollbar, range input, etc. */
...
`}</style>
      <div ref={mainDivRef} className={`draggable-area ${mainCardClasses}`} style={wrapperStyle}>
        ...
        <button
          ...
          className={`absolute top-4 right-4 ${panelBg} rounded-2xl p-2 shadow-lg cursor-pointer ${
            animationsEnabled ? 'hover:scale-105 transition-transform' : ''
          } z-20`}
          onClick={() => {
            // اول مخفی کردن ویجت در کامپوننت
            setIsWidgetVisible(false);

            // ارسال پیام به main process برای بستن پنجره
            const { ipcRenderer } = window.require ? window.require('electron') : {};
            if (ipcRenderer) {
        ...
        <div className={`w-full p-6 flex flex-col items-center relative md:border-r ${glassTheme ? 'border-white/20' : lightBg ? 'border-neutral-200' : 'border-neutral-800'}`}>
            <div role="button" className={`flex-grow flex flex-col w-full text-center ${notifications.length > 0 && showNotifications && !dnd ? 'justify-start pt-6' : 'justify-center items-center'}`}>
        ...
        <div className="w-full p-5 flex flex-col gap-3">
          ...
          <div className="mt-2">
            ...
            <div role="button" className="grid grid-cols-7 gap-1">
           ...
           <div className="flex-grow flex flex-col justify-end min-h-[170px]">
                <div className={`transition-all duration-300 ease-in-out ${selectedDate ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 invisible'}`}>
                    {selectedDate && (
                        <div role="button" className={`${panelBg} mt-2 rounded-2xl p-3 flex flex-col gap-2`}>
                        ...
                        <div className={`${panelBg} rounded-2xl p-3`}>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {isEditingCity ? (
                                    ...
                                    <div role="button" className={`text-sm cursor-pointer`} style={smallTextStyle} onClick={() => setIsEditingCity(true)}>{weather.loading ? '...' : settings.weatherCity}</div>
                                )}
                            </div>
                            {weather.loading ? (
